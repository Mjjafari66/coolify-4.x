...
/**
 * Drop user-supplied proxy labels that can collide across applications.
 *
 * Traefik merges routers/services with identical names coming from different
 * containers into one load balancer, which makes requests round-robin between
 * unrelated applications. Only labels whose router/service name contains the
 * application uuid are kept. Labels with unresolved ${VAR} placeholders are
 * dropped as well because compose variables are never expanded in labels.
 */
function filterUnsafeComposeProxyLabels(Collection $labels, string $uuid): Collection
{
    return $labels->filter(function ($label) use ($uuid) {
        if (! is_string($label)) {
            return true;
        }
        if (preg_match('/\$\{[^}]+\}/', $label)) {
        ...
        if (preg_match('/^traefik\.http\.(routers|services)\.([^.]+)\./', $label, $matches)) {
            return str_contains($matches[2], $uuid);
        }
...
function inferComposeServicePort(array $service): ?int
{
    foreach (data_get($service, 'expose', []) as $expose) {
        if (is_numeric($expose)) {
            return (int) $expose;
        }

        if (is_string($expose) && preg_match('/^(\d+)/', $expose, $matches)) {
            return (int) $matches[1];
        }
    }

    foreach (data_get($service, 'ports', []) as $port) {
        if (! is_string($port)) {
            continue;
        }

        $target = str_contains($port, ':') ? last(explode(':', $port)) : $port;
...
function extractHttpHealthcheckUrl(mixed $test): ?string
{
    if (is_string($test)) {
        $command = $test;
    } elseif (is_array($test)) {
        $parts = $test;
        if (isset($parts[0]) && in_array(strtoupper((string) $parts[0]), ['CMD', 'CMD-SHELL'], true)) {
            array_shift($parts);
        }
    ...
    if (! preg_match('#https?://[^\s\'"]+#i', $command, $matches)) {
        return null;
    }
...
function buildHttpHealthcheckShell(string $url, string $method = 'GET'): string
{
    $escapedUrl = escapeshellarg($url);
    $escapedMethod = escapeshellarg($method);
    $pythonUrl = str_replace("'", "\\'", $url);

    return "curl -s -X {$escapedMethod} -f {$escapedUrl} > /dev/null 2>&1 || wget -q -O- {$escapedUrl} > /dev/null 2>&1 || python3 -c \"import urllib.request; urllib.request.urlopen('{$pythonUrl}')\" || exit 1";
}

function normalizeComposeHealthcheck(array $service): array
{
    ...
    $testString = is_array($test) ? implode(' ', array_map('strval', $test)) : (string) $test;

    if (! str($testString)->upper()->contains(['CURL', 'WGET', 'HTTP://', 'HTTPS://'])) {
        return $service;
    }

    if (str_contains($testString, 'python3 -c') || str_contains($testString, 'urllib.request')) {
        return $service;
    }

    $url = extractHttpHealthcheckUrl($test);
    if ($url === null) {
        return $service;
    }

    $service['healthcheck']['test'] = ['CMD-SHELL', buildHttpHealthcheckShell($url)];

    return $service;
}

function ensureComposeApplicationHealthcheck(array $service): array
{
    if (data_get($service, 'exclude_from_hc')) {
        return $service;
    }

    $port = inferComposeServicePort($service);
    $existingTest = data_get($service, 'healthcheck.test');

    if ($existingTest !== null) {
        return normalizeComposeHealthcheck($service);
    }
    ...
    $service['healthcheck'] = array_merge([
        'interval' => '30s',
        'timeout' => '10s',
        'retries' => 3,
        'start_period' => '60s',
    ], data_get($service, 'healthcheck', []), [
        'test' => ['CMD-SHELL', buildHttpHealthcheckShell("http://127.0.0.1:{$port}/")],
    ]);
...
function fqdnLabelsForTraefik(string $uuid, Collection $domains, bool $is_force_https_enabled = false, $onlyPort = null, ?Collection $serviceLabels = null, ?bool $is_gzip_enabled = true, ?bool $is_stripprefix_enabled = true, ?string $service_name = null, bool $generate_unique_uuid = false, ?string $image = null, string $redirect_direction = 'both', bool $is_http_basic_auth_enabled = false, ?string $http_basic_auth_username = null, ?string $http_basic_auth_password = null, ProxySslMode $proxy_ssl_mode = ProxySslMode::Letsencrypt, ?array $manual_ssl_hosts = null)
{
    ...
    foreach ($domains as $loop => $domain) {
        try {
            ...
            $schema = $url->getScheme();
            $port = $url->getPort();
            if ($proxy_ssl_mode === ProxySslMode::Manual && $manual_ssl_hosts !== null && $schema === 'https') {
                if (! in_array($host, $manual_ssl_hosts, true)) {
                    $schema = 'http';
                }
            }
            if (is_null($port) && ! is_null($onlyPort)) {
            ...
            if ($schema === 'https' && $proxy_ssl_mode !== ProxySslMode::Off) {
                ...
                if ($proxy_ssl_mode->usesLetsEncrypt()) {
...
function generateLabelsApplication(Application $application, ?ApplicationPreview $preview = null): array
{
    ...
    $manualSslHosts = $proxySslMode === ProxySslMode::Manual ? $application->manualSslHosts() : null;
    $labels = collect([]);
    if ($pull_request_id === 0) {
        if ($application->fqdn) {
...
/**
 * Resolve deployed compose file basename and location from application settings.
 *
 * @return array{basename: string, location: string}
 */
function resolveDeployedComposeFile(string $dockerComposeLocation, int $pullRequestId = 0): array
{
    $basename = ltrim($dockerComposeLocation, '/');
    ...
    if ($pullRequestId !== 0) {
        $extension = pathinfo($basename, PATHINFO_EXTENSION) ?: 'yaml';
        $filename = pathinfo($basename, PATHINFO_FILENAME) ?: 'docker-compose';
        $directory = pathinfo($basename, PATHINFO_DIRNAME);
        $previewBasename = addPreviewDeploymentSuffix($filename, $pullRequestId).'.'.$extension;
        $basename = ($directory !== '.' ? $directory.'/' : '').$previewBasename;
    }
