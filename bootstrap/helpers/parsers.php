...
function applicationParser(Application $resource, int $pull_request_id = 0, ?int $preview_id = null, ?string $commit = null): Collection
{
    ...
    foreach ($services as $serviceName => $service) {
        ...
        $predefinedPort = inferComposeServicePort($service);
        ...
        $labels = filterUnsafeComposeProxyLabels($labels, $resource->uuid);
        ...
        if (! $isDatabase && $fqdns instanceof Collection && $fqdns->count() > 0) {
            ...
            [$proxySslMode, $manualSslHosts] = traefikSslOptionsForApplication($originalResource);
        ...
        if (! $isDatabase) {
            $service = ensureComposeApplicationHealthcheck($service);
        }
...
function serviceParser(Service $resource): Collection
{
    ...
    foreach ($services as $serviceName => $service) {
        ...
        if (! $isDatabase && $fqdns instanceof Collection && $fqdns->count() > 0) {
            ...
            $proxySslMode = serverProxySslMode($resource->server);
            if ($shouldGenerateLabelsExactly) {
                switch ($server->proxyType()) {
                    case ProxyTypes::TRAEFIK->value:
                        // SaaS edge is Cloudflare Flexible SSL (origin HTTP :80).
                        // Force-HTTPS Traefik redirects cause infinite 302 loops for customers.
                        $serviceLabels = $serviceLabels->merge(fqdnLabelsForTraefik(
                            uuid: $uuid,
                            domains: $fqdns,
