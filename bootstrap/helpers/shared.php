                        ...
                        if ($shouldGenerateLabelsExactly) {
                            switch ($resource->server->proxyType()) {
                                case ProxyTypes::TRAEFIK->value:
                                    // Cloudflare Flexible: origin speaks HTTP — do not force HTTPS redirect.
...
function coolifyImageWithVersion(string $image, string $version): string
{
    $lastSlash = strrpos($image, '/');
    $namePart = $lastSlash === false ? $image : substr($image, $lastSlash + 1);
