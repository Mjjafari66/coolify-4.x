...
class Server extends BaseModel
{
   ...
   redir $redirect_url
    ...
    respond 503
                ...
                $proxySslMode = serverProxySslMode($this);
                $catchallEntryPoints = $proxySslMode === ProxySslMode::Off ? ['http'] : ['http', 'https'];
                $dynamic_conf = [
                    'http' => [
                        'routers' => [
                            'catchall' => [
                                'entryPoints' => $catchallEntryPoints,
                                'service' => 'noop',
                                'rule' => 'PathPrefix(`/`)',
                                'priority' => -1000,
                            ],
                ...
                if ($proxySslMode !== ProxySslMode::Off) {
                    $dynamic_conf['http']['routers']['catchall']['tls'] = traefikRouterTlsConfig($this);
                }
                if (filled($redirect_url)) {
    ...
    public function setupDynamicProxyConfiguration()
    {
        ...
        if ($this->proxyType() === ProxyTypes::TRAEFIK->value) {
            ...
            if (empty($settings->fqdn) || (isCloud() && $this->id !== 0) || ! $this->isLocalhost()) {
                ...
                if ($schema === 'https' && serverProxySslMode($this) !== ProxySslMode::Off) {
                    $traefik_dynamic_conf['http']['routers']['coolify-http']['middlewares'] = [
                        0 => 'redirect-to-https',
                    ];

                    $tlsConfig = traefikRouterTlsConfig($this);
...
$schema://$host {
    ...
    public function proxySslMode(): ProxySslMode
    {
        return serverProxySslMode($this);
    }

    public function proxyCertificate()
    {
        return $this->sslCertificates()->where('is_proxy_certificate', true)->first();
    }
