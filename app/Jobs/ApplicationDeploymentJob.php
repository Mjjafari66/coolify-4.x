...
class ApplicationDeploymentJob implements ShouldBeEncrypted, ShouldQueue
{
    ...
    private function write_deployment_configurations()
    {
        ...
        if (isset($this->docker_compose_base64)) {
            ...
            $deployedComposeFile = resolveDeployedComposeFile($this->docker_compose_location, $this->pull_request_id);
            $composeFileName = "$mainDir/{$deployedComposeFile['basename']}";
            if ($this->pull_request_id !== 0) {
                $this->docker_compose_location = $deployedComposeFile['location'];
            }
            $this->execute_remote_command(
                [
                    "mkdir -p $mainDir",
    ...
    {
        $this->checkForCancellation();
        $helperImage = coolifyImageWithVersion(config('constants.coolify.helper_image'), getHelperVersion());
...
COPY ./nginx.conf /etc/nginx/conf.d/default.conf");
    ...
    private function generate_healthcheck_commands()
    {
        ...
        return buildHttpHealthcheckShell("{$scheme}://{$host}:{$health_check_port}".($path ?? '/'), $method);
    }
